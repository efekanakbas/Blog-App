...
const Main: React.FC<MainProps> = ({messages}) => {
  ...
  return (
    <Box   className='w-[73%] h-full p-4 pt-0  bg-white ' sx={{borderRadius:'0 20px 20px 0'}}>
  ...
  messages?.map((item, i, array) => (
    <Box className={`flex px-8 relative ${item.me ? ' justify-end' : 'justify-start '}`} key={i}>
      ...
      className={`flex p-4 rounded-2xl flex-col max-w-[75%] ${item.me ? 'bg-blue-600 text-white flex-row-reverse ' : 'bg-white '}`}
    ...
    >
      {item.message}
            ...
            <figure className='scale-[2.7]'>
              <Avatar src='images/avatars/6.png' />
            </figure>
            <Typography className='mt-12 font-bold'>
              Mustafa Turan
            </Typography>
          ...
          <Box className='bg-white absolute bottom-0 w-[97%] h-[80px] mb-4 rounded-2xl'>
            ...
            <Input size='medium' className='w-full px-20 mt-3' paddingLeft={false} autoFocus={false} value={values.inputValue} handleChange={handleChange} name='inputValue' type='text' placeholder='Type here...'  />
            </form>
          </Box>
      </Box>
    </Box>
